		...
		var mdater = {
			...
			getDateStr : function(y, m, d){
				...
				for(var j=week-1; j>=0; j--){
					dayStr += '<li onclick="" class="prevdate" data-day="'+(lastMonthDays-j)+'"><span>'+(lastMonthDays-j)+'</span></li>';
				}
				//再补上本月的所有天;
				...
				for(var j=startDay; j<=endDay; j++){
					...
					dayStr += '<li onclick="" class="dayClick '+current+'" data-day="'+j+'"><span>'+j+'</span></li>';
				}
				...
				if(nextMonthStartWeek!==0){
					for(var i=1; i<=7-nextMonthStartWeek; i++){
						dayStr += '<li onclick="" class="nextdate" data-day="'+i+'"><span>'+i+'</span></li>';
					}
				}
